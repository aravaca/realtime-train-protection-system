...
이 풀스택 프로젝트는 JR East E233를 모델로 한 정차 훈련용 시뮬레이터입니다.
프론트엔드(HTML/CSS/JS) + 백엔드(FastAPI WebSocket)를 통합하여, 브레이크 내비게이션 캔버스(거리-속도 곡선)와 **TASC(자동 정차 보조)**를 활용한 실시간 제동 연습과 피드백/점수 평가를 제공합니다.
...
데드밴드 & 최소 유지시간으로 헌팅 방지, 승차감 향상

수동 개입(키/터치) 시 즉시 Manual로 전환
...
정차 정확도↑ / 승차감 일관성↑: 데드밴드와 홀드 타임으로 불필요한 노치 헌팅을 방지, 항상 비슷한 프로파일로 제동

훈련 효율↑:

브레이크 내비 캔버스로 “곡선과 빨간선이 만날 때”의 타이밍 감각을 체득

피드백/점수로 본인의 습관을 정량적으로 점검


교육/검증용: 시나리오(거리/구배/속도/마찰) 다양화, 승차감(저크)과 정차 정밀도를 같이 평가
...
노치별 제동 곡선들이 표시되고, **빨간 세로선(남은 거리)**과의 교차가 “제동/완해 타이밍”
...
3. 브라우저에서 접속
http://localhost:8000/ → HUD/오버레이 UI가 열립니다.
...
계단 제동 (build):
현재 노치에서 계산한 정지거리 s_cur가 (rem - 데드밴드) 보다 크면 한 단계 강화(↑)
충분해지면 완해 단계로 전환

계단 완해 (relax):
한 단계 아래 노치의 정지거리 s_dn이 (rem + 데드밴드) 이하면 한 단계 완해(↓)
이를 반복, B1으로 마무리
...
저속/근거리에서 B1 바이어스로 승차감 개선(필요 시 미세 튜닝 가능)
...
mass_t: 1량 기준 질량 (총질량은 편성 수 × 탑승률로 실시간 갱신)


scenario.json

L(정차 목표까지 거리), v0(초기 속도), grade_percent, mu(마찰)
...
브라우저 캐시/서비스워커/CloudFront 캐시를 클리어

Nginx/EC2에서 static/ 파일 최신 배포 확인


HUD에 EB 곡선 미표시:

vehicle.notches/notch_accels 길이와 프론트의 렌더 순서를 점검

서버는 [N..EB] 배열을 반전해 사용함


TASC가 완해를 안 함:

tasc_deadband_m을 0.3~0.8 범위에서 조정

tasc_hold_min_s를 0.2~0.35로 조정(너무 작으면 헌팅, 너무 크면 둔감)
...
곡선/정차 위치 학습(ML) 기반의 TASC 적응형 파라미터 튜닝
...
0cm 정차 보너스를 노리며 저크 점수도 신경 쓰면, 정확도+승차감 둘 다 잡을 수 있어요.
